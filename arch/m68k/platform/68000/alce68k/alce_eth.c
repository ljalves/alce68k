...
#include "../../../arch/m68k/include/asm/alce68k.h"

#include "alce_eth.h"

#define DRV_NAME	"alce-eth"
...
spi_read_buf(struct enc28j60_net *priv, int len, u8 *data)
{
	...
	// assert CS
	ETHSPI_CTRL = SPICTRL_EN | SPICTRL_CS;

	ETHSPI_DATA = ENC28J60_READ_BUF_MEM;
	while (ETHSPI_CTRL & SPICTRL_BUSY);
			 ...
			 const u8 *data)
{
	...
	ETHSPI_DATA = ENC28J60_WRITE_BUF_MEM;
	while (ETHSPI_CTRL & SPICTRL_BUSY);
 ...
 * Register byte read
 */
static u8 nolock_regb_read(struct enc28j60_net *priv,
				     u8 address)
{
...
static u8 locked_regb_read(struct enc28j60_net *priv,
				     u8 address)
{
	u8 ret;
 ...
 * Register word read
 */
static u16 nolock_regw_read(struct enc28j60_net *priv,
				     u8 address)
{
	u8 rl;
	u16 rh;
...
static u16 locked_regw_read(struct enc28j60_net *priv,
				     u8 address)
{
	u16 ret;
...
static int enc28j60_probe(struct platform_device *pdev)
{
	struct net_device *ndev;
	struct enc28j60_net *priv;
	
	//struct alcenet_platform_data *pdata;
	//struct resource *r;
	//static int dev_id;
	
	int ret = 0;

	printk("ENC28J60: Probe\n");
	...
	ndev = alloc_etherdev(sizeof(struct enc28j60_net));
	if (!ndev) {
		/*if (netif_msg_drv(&debug))
			dev_err(&spi->dev, DRV_NAME
				": unable to alloc new ethernet\n");*/
		printk("ENC28J60: Enable to allocate new ethernet\n");
		ret = -ENOMEM;
		goto error_alloc;
	}
	...
	priv->netdev = ndev;	/* priv to netdev reference */
	...
	if (!enc28j60_chipset_init(ndev)) {
		/*if (netif_msg_probe(priv))
			dev_info(&spi->dev, DRV_NAME " chip not found\n");*/
		printk(DRV_NAME " chip not found\n");
		ret = -EIO;
		goto error_irq;
	}
	random_ether_addr(ndev->dev_addr);
	enc28j60_set_hw_macaddr(ndev);

	/* Board setup must set the relevant edge trigger type;
	 * level triggers won't currently work.
	 */
	ret = request_irq(INT_ENC28J60, enc28j60_irq, 0, DRV_NAME, priv);
	if (ret < 0) {
		if (netif_msg_probe(priv))
			/*dev_err(&priv->netdev, DRV_NAME ": request irq %d failed "
				"(ret = %d)\n", spi->irq, ret);*/
			printk(DRV_NAME ": request irq %d failed "
				"(ret = %d)\n", INT_ENC28J60, ret);
		goto error_irq;
	}
	...
	//dev->irq = priv->irq;
	ndev->netdev_ops = &enc28j60_netdev_ops;
	ndev->watchdog_timeo = TX_TIMEOUT;
	SET_ETHTOOL_OPS(ndev, &enc28j60_ethtool_ops);
	...
	if (ret) {
		if (netif_msg_probe(priv))
			/*dev_err(&spi->dev, "register netdev " DRV_NAME
				" failed (ret = %d)\n", ret);*/
			printk("register netdev " DRV_NAME
				" failed (ret = %d)\n", ret);
		goto error_register;
	}
...
static int enc28j60_remove(struct platform_device *pdev)
{
...
static struct platform_driver enc28j60_driver = {
	.driver	= {
		.name    = "enc28j60",
		.owner	 = THIS_MODULE,
	},
...
static int __init enc28j60_init(void)
{
	...
	return platform_driver_register(&enc28j60_driver);
}

static void __exit enc28j60_cleanup(void)
{
	platform_driver_unregister(&enc28j60_driver);
}
